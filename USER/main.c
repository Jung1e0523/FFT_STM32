...
int main() {
    ...
    u32 fft_max = 0;
    float adc_max = 0.0;
    u16 fre[22] = {100, 150, 200, 250,  300,  350,  400,  450,  500,  550,  600,
                 700, 800, 900, 1000, 1200, 1500, 2000, 3000, 4000, 5000, 6000};
    delay_init();    // 延时函数初始化
    ...
    while (1) {
        ...
        case KEY0_PRES: {
            ...
            switch (x) {
                case hpass_100:
                    PAout(1) = 0;
                    PAout(2) = 0;
                    LCD_ShowxNum(40, 0, 100, 4, 24, 0);
                    break;
                case hpass_200:
                    PAout(1) = 0;
                    PAout(2) = 1;
                    LCD_ShowxNum(40, 0, 200, 4, 24, 0);
                    break;
                case hpass_400:
                    PAout(1) = 1;
                    PAout(2) = 0;
                    LCD_ShowxNum(40, 0, 400, 4, 24, 0);
                    break;
                default:
                    PAout(1) = 0;
                    PAout(2) = 0;
        ...
        case KEY1_PRES:
            ...
            switch (y) {
                ...
                case lpass_800:
                    ...
                    PAout(4) = 0;
                    PAout(5) = 1;
                    LCD_ShowxNum(150, 0, 800, 4, 24, 0);
                    break;
                case lpass_1000:
                    PAout(3) = 0;
                    PAout(4) = 1;
                    PAout(5) = 0;
                    LCD_ShowxNum(150, 0, 1000, 4, 24, 0);
                    break;
                case lpass_1500:
                    PAout(3) = 0;
                    PAout(4) = 1;
                    PAout(5) = 1;
                    LCD_ShowxNum(150, 0, 1500, 4, 24, 0);
                    break;
                case lpass_6000:
                    PAout(3) = 1;
                    PAout(4) = 0;
                    PAout(5) = 0;
                    LCD_ShowxNum(150, 0, 6000, 4, 24, 0);
                    break;
                default:
                    PAout(3) = 0;
                    PAout(4) = 0;
        ...
        for (i = 1; i < 22; i++) {
            if (temp_2[i] > temp_1[i])
                LCD_Fill(34 * i + 10, 440 - temp_2[i], 34 * i + 30, 440, GREEN);
                // 本轮数据比上一帧数据大，在上一帧的基础上增加画点；
            else
                LCD_Fill(34 * i + 10, 40, 34 * i + 30, 440 - temp_2[i], BLACK);
                // 本轮数据比上一帧数据小，在上一帧的基础上画黑点覆盖，以求达到降低条形的效果；
        }
